...
# set path
data_path = "./frame/" # define spatial data path
action_name_path = "./action.pkl"   # load preprocessed action names
save_model_path = "./models/"  # save Pytorch models
test_path = "./validate_frame/"  # frames of validation/test
...
learning_rate = 1e-4
log_interval = 10
img_x, img_y = 240, 320  # video 2d frame size
...
def train(log_interval, model, device, train_loader, optimizer, epoch):
    ...
    # with torch.no_grad(): # over memory
...
# validate setting
v_names = os.listdir(test_path)
v_actions = []
v_all_names = []
for v in v_names:
    loc1 = v.find('v_')
    loc2 = v.find('_g')
    v_actions.append(v[(loc1 + 2): loc2])
    v_all_names.append(v)

test_list = v_all_names
test_label = labels2cat(le, v_actions)
...
train_set, valid_set = Dataset_3DCNN(data_path, train_list, train_label, selected_frames, transform=transform), \
                       Dataset_3DCNN(test_path, test_list, test_label, selected_frames, transform=transform)
